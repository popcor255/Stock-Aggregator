...
var options = null;
var stock_list = ["GOOGL", "TSLA", "FB", "JNJ", "SPY"];
var loaded = new Set();
var stock_index = 0;
...
function getResults(index) {
  window.setTimeout(function() {
    if (stock_index < stock_list.length) {
      addChart("candlestick", stock_list[index]);
    }
  }, 200);
  return results;
}
...
function createChart(type, label) {
  ...
  charts[chart_index] = new Chart(
    document.getElementById("chart_" + chart_index),
    obj
  );
